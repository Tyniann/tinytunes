...
class GoogleDriveCloudLibrarySource implements CloudLibrarySource {
  ...
  ///
  /// New downloads land in `[cacheRootDirectory]/gdrive/<fileId>/…`. Legacy
  /// files directly under [cacheRootDirectory]/`<fileId>/` are still resolved.
  ...
  Directory get _providerCacheRoot => Directory(
    p.join(_cacheRoot.path, CloudProviderId.googleDrive.cacheDirectoryName),
  );
  ...
  Future<MediaLocator> downloadToCache(
    ...
    final destDir = Directory(p.join(providerRoot.path, fileId));
  ...
  ///
  /// Prefers the provider subdirectory; falls back to the legacy flat layout.
  Future<String?> cachedFilePath(MediaLocator item) async {
    final fileId = DriveMediaLocator.decode(item);
    final modern = await _firstFileIn(
      Directory(p.join(_providerCacheRoot.path, fileId)),
    );
    if (modern != null) return modern;
    return _firstFileIn(Directory(p.join(_cacheRoot.path, fileId)));
  }
